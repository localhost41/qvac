...
The SDK returns raw, mono, signed 16-bit PCM samples as a plain `number[]`. The `buffer` promise resolves to the complete sample array, while `bufferStream` yields individual samples as they become available. The data has no WAV header or other container metadata; use the [example utility](#utils) to write it as a WAV file.
...
| ----------------------- | --------------------------------------------------------------------------------------------------- |
| Chatterbox              | 24,000 Hz                                                                                           |
| Supertonic              | 44,100 Hz, or `modelConfig.outputSampleRate` when set                                               |
| Parler                  | 44,100 Hz, or `modelConfig.outputSampleRate` for non-streaming output; streaming requires 44,100 Hz |
| LavaSR enhancer enabled | 48,000 Hz by default; Supertonic can override it with `modelConfig.outputSampleRate`                |
