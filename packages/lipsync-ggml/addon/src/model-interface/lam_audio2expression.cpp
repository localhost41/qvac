...
bool
LamAudio2Expression::load(const std::string& ggufPath, ggml_backend_t backend) {
  if (backend != nullptr) {
    backend_ = backend;
    ownsBackend_ = false;
  } else {
    // Resolve the CPU backend through the device registry rather than
    // ggml_backend_cpu_init(): under GGML_BACKEND_DL the CPU backend is a
    // runtime-loaded plugin and the direct symbol is not exported.
    ggml_backend_dev_t cpuDev =
        ggml_backend_dev_by_type(GGML_BACKEND_DEVICE_TYPE_CPU);
    backend_ = cpuDev != nullptr ? ggml_backend_dev_init(cpuDev, nullptr)
                                 : nullptr;
    ownsBackend_ = true;
    if (backend_ != nullptr) {
      ggml_backend_reg_t reg = ggml_backend_dev_backend_reg(cpuDev);
      auto* setThreads = reinterpret_cast<void (*)(ggml_backend_t, int)>(
          ggml_backend_reg_get_proc_address(reg,
                                            "ggml_backend_set_n_threads"));
      if (setThreads != nullptr) {
        const unsigned hw = std::thread::hardware_concurrency();
        setThreads(backend_, static_cast<int>(hw > 2 ? hw - 2 : 1));
      }
    }
  }
  if (backend_ == nullptr) {
