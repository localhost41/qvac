...
namespace lipsync_backend_selection {
...
ggml_backend_dev_t pickBestGpuDevice() {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (adreno > 0) {
      ...
      if (isOpenCl && adreno >= 800) {
        QLOG_IF(
            Priority::INFO,
            "lipsync_backend_selection: Adreno " + std::to_string(adreno) +
      ...
      QLOG_IF(
          Priority::WARNING,
          "lipsync_backend_selection: skipping Adreno " + std::to_string(adreno) +
              " " + backendName +
    ...
    QLOG_IF(
        Priority::INFO,
        "lipsync_backend_selection: non-Adreno GPU accepted: " + desc +
            " (backend: " + backendName + ")");
  ...
  if (hipDev != nullptr) {
    QLOG_IF(
        Priority::INFO,
        "lipsync_backend_selection: preferring HIP/ROCm GPU (Vulkan is fallback)");
    return hipDev;
  }
  return fallbackGpu;
}
