...
test(
  ...
  async (t) => {
    if (isMobile) {
      // Parler is not provisioned on mobile Device Farm yet (mini q8_0 is
      // ~1.16 GB); passing skip keeps the mobile group green.
      t.pass('parler unprovisioned on mobile — skipped')
      return
    }
    const baseDir = getBaseDir()
