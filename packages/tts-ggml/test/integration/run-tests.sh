...
# (e.g. after a cancelled/aborted job on macOS), so run-with-exit.js writes an
# .exit-code file when tests finish.  This script monitors for that file and
# terminates bare cleanly, and force-kills the process if a hang blows past
# INTEGRATION_TEST_TIMEOUT instead of burning the whole CI job budget.
