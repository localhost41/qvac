...
test('Parler: ttsParams shape forwards the full config surface', (t) => {
  ...
  t.absent(params.useGPU, 'useGPU never forwarded for parler (CPU-only)')
...
test('Parler: constructor guards reject conflicting / unsupported options', (t) => {
  ...
  t.exception(
    () => createMockedParlerModel({ extra: { streamChunkTokens: 25 } }),
    /Chatterbox-only/,
    'streamChunkTokens throws'
  )
  t.exception(
    () =>
  ...
  t.exception(
    () => createMockedParlerModel({ extra: { config: { useGPU: true } } }),
    /CPU-only/,
    'useGPU: true throws'
  )
  t.exception(
    () => createMockedParlerModel({ extra: { nGpuLayers: 99 } }),
    /CPU-only/,
    'nGpuLayers > 0 throws'
  )
})
...
test('Parler: reload merges description/template + sampling knobs', async (t) => {
  ...
  // reload cannot smuggle GPU intent past the constructor guard.
  await t.exception(model.reload({ useGPU: true }), /CPU-only/, 'reload({useGPU:true}) rejects')
  await model.unload()
})
