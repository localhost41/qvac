...
// Parler family: mini/large ship q8_0 + q6_k, indic ships f32/f16/q8_0.
// One generous size band covers every published tier (q6_k mini ~0.98 GB up
// to indic f32 ~3.67 GB); the ~50%-headroom convention of the other bands.
const REGISTRY_DATE_PARLER = '2026-07-17'
const SIZE_PARLER = { minSize: 500_000_000, maxSize: 5_500_000_000 }
const PARLER_PUBLISHED = {
  mini: { file: 'parler-mini-v1', tiers: ['q8_0', 'q6_k'] },
  large: { file: 'parler-large-v1', tiers: ['q8_0', 'q6_k'] },
...
function parlerGguf(variant, quant) {
  ...
  if (pub.tiers.includes(quant)) {
    gguf.registryPath = `qvac_models_compiled/ggml/parler/${REGISTRY_DATE_PARLER}/${pub.file}-${quant}.gguf`
    gguf.registrySource = REGISTRY_SOURCE
  }
  return gguf
}
