...
function validateRunInput (input, hparams) {
  ...
  for (let i = 0; i < input.images.length; i++) {
    ...
    if (imagesArePatches) {
      ...
      if (!patchElemsKnown) {
        throw new QvacErrorAddonVla({ code: ERR_CODES.INVALID_INPUT, adds: `input.images[${i}] (patches): addon did not surface hparams.imagePatchElems, cannot validate patch buffer length` })
      }
      if (img.length !== patchElems) {
        throw new QvacErrorAddonVla({ code: ERR_CODES.INVALID_INPUT, adds: `input.images[${i}] (patches) length ${img.length} != ${patchElems}` })
      }
